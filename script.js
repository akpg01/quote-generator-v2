...
let selectedCategory = localStorage.getItem("selected") || "all";
let apiQuotes = [];
let apiCategories = [];

let listOfCat = categories.sort();

// Show quote based on category
    ...
    for (i = 0; i < arr.length; i++) {
      ...
      if (arr[i].substr(0, val.length).toUpperCase() == val.toUpperCase()) {
        ...
        b.addEventListener("click", function (e) {
          ...
          // save value selected in dropdown list to local storage
          localStorage.setItem("selected", inp.value);
          getFilteredArray();
...
async function getCategories() {
  ...
  try {
    ...
    console.log("yo, i got here....", apiCategories.tags);
    let tags = [];
    apiCategories.tags.forEach((e) => tags.push(e.name));
    console.log("these are downloaded apicategories: ", tags.sort());
...
async function getQuotes() {
  ...
  try {
    ...
    //console.log("this apiQuotes: ", apiQuotes);
    newQuote();
  } catch (error) {
    // If an error in connection occurs, default quotes hardcoded file.
    apiQuotes = quotes;
    newQuote();
  }
}
...
console.log("this is categories in here....", categories);

// On Load
getCategories();
autocomplete(inputBox, categories);
getQuotes();
